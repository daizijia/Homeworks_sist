...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ones = torch.ones_like(scores_real).type(dtype)
    loss_real = 0.5 * (scores_fake ** 2 + (scores_real - ones) ** 2)
    loss = loss_real.mean()
...
def ls_generator_loss(scores_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ones = torch.ones_like(scores_fake).type(dtype)
    loss = 0.5 * (scores_fake - ones) ** 2
    loss = loss.mean()
