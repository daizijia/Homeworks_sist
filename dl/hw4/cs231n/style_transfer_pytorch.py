...
def content_loss(content_weight, content_current, content_original):
    ...
    loss = content_weight * torch.sum((content_current.view(C_l, H_l * W_l) - 
                                       content_original.view(C_l, H_l * W_l))**2)
    return loss
...
def gram_matrix(features, normalize=True):
    ...
    if normalize is True:
      gram = f.matmul(f.permute(0, 2, 1)) / (H * W * C)
    else:
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        gram_feats = gram_matrix(feats[style_layers[i]].clone())
        loss += style_weights[i] * torch.sum((gram_feats - style_targets[i])**2)
    return loss
