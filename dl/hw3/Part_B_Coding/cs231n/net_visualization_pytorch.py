...
def compute_saliency_maps(X, y, model):
    ...
    pre = model(X)
    score = pre.gather(1, y.view(-1,1)).squeeze()
    score.backward(torch.ones(score.shape))
...
def make_fooling_image(X, target_y, model):
    ...
    for iteration in range(100):
        ...
        _, pred = scores.data.max(dim = 1)
        if pred[0] == target_y:
            break
        target = scores[0, target_y]
        target.backward()
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    grad = img.grad.data - l2_reg * img.data
