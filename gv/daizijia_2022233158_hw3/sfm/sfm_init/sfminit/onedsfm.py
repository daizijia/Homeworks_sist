...
def oneDSfM(edges, poses, num_samples=48, weights_file=None,
            threshold=0.10, num_workers=4, verbose=False):
    """
...
def kdeSample(x, y, samples):
    """
    TO DO: Take 2D vectors x and y, fit a kernel density estimate to them, and
    resample new points.
    """

    random.seed(311)
    input_downsamples = 2000

    if len(x) > input_downsamples:
        indices = random.sample(range(len(x)), input_downsamples)
        x_downsampled = [x[i] for i in indices]
        y_downsampled = [y[i] for i in indices]
    else:
        x_downsampled, y_downsampled = x, y

    data_points = np.column_stack((x_downsampled, y_downsampled))

    kde = scipy.stats.kde.gaussian_kde(data_points.T)
    sampleDirs = kde.resample(size=samples).T
